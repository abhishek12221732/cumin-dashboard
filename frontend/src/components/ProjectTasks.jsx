...
function ProjectTasks() {
  ...
  const fetchTasks = async () => {
    ...
    let url = `/items/projects/${selectedProject.id}/items`;
  ...
  const handleEditSave = async () => {
    ...
    await import('../utils/api').then(m => m.apiFetch(`/items/${editTaskId}`, {
      method: 'PATCH',
  ...
  const handleDelete = async (id) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        const token = localStorage.getItem('token');
        try {
          const res = await import('../utils/api').then(m => m.apiFetch(`/items/${id}`, {
            method: 'DELETE'
          }));
