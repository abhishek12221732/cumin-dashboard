...
export default function TaskDetailModal({ isOpen, onRequestClose, taskId }) {
  ...
  const fetchTask = async () => {
    setLoading(true);
    try {
      const taskRes = await import('../utils/api').then(m => m.apiFetch(`/items/${taskId}`));
  ...
  const handleSave = async () => {
    try {
        ...
        await import('../utils/api').then(m => m.apiFetch(`/items/${taskId}`, {
            method: 'PATCH',
  ...
  const handleAddComment = async () => {
    ...
    const res = await import('../utils/api').then(m => m.apiFetch(`/items/${task.id}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
