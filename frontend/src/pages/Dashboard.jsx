...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        // Fetch stats
        const statsRes = await apiFetch('/dashboard/stats', {}, navigate);
        ...
        // Fetch activity log
        const activityRes = await apiFetch('/items/activity', {}, navigate);
        ...
        // Fetch my tasks
        const myTasksRes = await apiFetch('/items/my-tasks', {}, navigate);
        ...
        const projectsRes = await apiFetch('/projects', {}, navigate);
        ...
        if (selectedProject) {
          const progressRes = await apiFetch(`/projects/${selectedProject.id}/progress`, {}, navigate);
  ...
  const fetchInvitations = async () => {
    ...
    try {
      const res = await apiFetch('/my-invitations', {}, navigate);
      if (res.status === 401) {
        localStorage.removeItem('token');
        localStorage.removeItem('user');
        navigate('/login');
  ...
  const handleInvitationAction = async (inviteId, projectId, action) => {
    ...
    try {
      const res = await apiFetch(`/projects/${projectId}/invitation/${inviteId}/${action}`, { method: 'POST' }, navigate);
      if (res.status === 401) {
        localStorage.removeItem('token');
        localStorage.removeItem('user');
        navigate('/login');
  ...
  useEffect(() => {
    ...
    const fetchForProject = async () => {
      ...
      // Fetch columns
      const colRes = await apiFetch(`/projects/${selectedTaskProject}/columns`, {}, navigate);
      ...
      // Fetch users
      const userRes = await apiFetch(`/projects/${selectedTaskProject}/members`, {}, navigate);
      ...
      // Fetch epics
      const epicsRes = await apiFetch(`/items/projects/${selectedTaskProject}/items`, {}, navigate);
  ...
  const fetchUsers = async () => {
    ...
    const res = await apiFetch(`/projects/${selectedProject.id}/members`, {}, navigate);
    const data = await res.json();
  ...
  const fetchColumns = async () => {
    ...
    const res = await apiFetch(`/projects/${selectedProject.id}/columns`, {}, navigate);
    if (res.ok) {
      const data = await res.json();
      setColumns(data.columns);
    }
  ...
  const handleTaskSubmit = async (values) => {
    ...
    const res = await apiFetch(`/items/projects/${selectedTaskProject}/items`, {
      method: 'POST',
