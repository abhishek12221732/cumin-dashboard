...
function Board() {
  ...
  const fetchColumns = async () => {
    ...
    const res = await apiFetch(`/projects/${selectedProject.id}/columns`);
    if (res.ok) {
      const data = await res.json();
  ...
  const createDefaultColumns = async () => {
    ...
    for (const col of defaultCols) {
      if (!existing.includes(col.status)) {
        await apiFetch(`/projects/${selectedProject.id}/columns`, {
          method: 'POST',
  ...
  const fetchTasks = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/items/projects/${selectedProject.id}/items`);
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDragEnd = async (event) => {
    ...
    const res = await apiFetch(`/items/${active.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const canEditOrMove = (task) => {
    ...
    if (selectedProject && selectedProject.manager_id === userId) return true;
