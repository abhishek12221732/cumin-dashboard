...
function Reports({ projectId }) {
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      setLoading(true);
      try {
        const res = await import('../utils/api').then(m => m.apiFetch(`/reports/project/${projectId}`));
  ...
  const fetchMembers = async () => {
    ...
    try {
      const res = await import('../utils/api').then(m => m.apiFetch(`/projects/${projectId}/members`));
      const data = await res.json();
