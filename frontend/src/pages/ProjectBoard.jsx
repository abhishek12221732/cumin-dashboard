...
function ProjectBoard() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await import('../utils/api').then(m => m.apiFetch('/me'));
        if (res.ok) {
  ...
  const fetchColumns = async () => {
    ...
    try {
      const res = await import('../utils/api').then(m => m.apiFetch(`/projects/${selectedProject.id}/columns`));
      if (res.ok) {
        const data = await res.json();
  ...
  const createDefaultColumns = async () => {
    ...
    for (const col of defaultCols) {
      if (!existing.includes(col.status)) {
        await import('../utils/api').then(m => m.apiFetch(`/projects/${selectedProject.id}/columns`, {
          method: 'POST',
  ...
  const fetchTasks = async () => {
    setLoading(true);
    try {
      const res = await import('../utils/api').then(m => m.apiFetch(`/items/projects/${selectedProject.id}/items`));
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDelete = async (itemId) => {
    await import('../utils/api').then(m => m.apiFetch(`/items/${itemId}`, { method: 'DELETE' }));
    fetchTasks();
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      const res = await import('../utils/api').then(m => m.apiFetch(`/items/${active.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
